...
> 当前稳定版：`v1.0.0`；可选测试版：`v1.1.0-beta.2`。短信与忽略电池优化会直接弹出系统确认；厂商启动管理没有普通 App 可调用的标准授权框，雁笺会打开最接近的系统页，并在用户返回后立即确认结果。正式版与 Beta 使用同一签名，可覆盖安装并保留本机配置。
...
4. 按“系统守护”逐项点按：短信权限和忽略电池优化会直接显示系统确认；厂商启动管理需要在打开的系统页中允许自启动、关联启动和后台活动，返回雁笺后确认一次即可。
5. 进入“锁屏试投”，用一条真实短信完成闭环，再分别验证亮屏、锁屏、Wi-Fi、移动网络和断网补发。
