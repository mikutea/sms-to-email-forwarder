...
> 当前版本：`v0.3.0-beta.4`（定稿视觉一致性版）。八个页面已按同一份“月白青玉”定稿逐屏实现并在 Android 15 虚拟设备上完成同画布对照，自动化测试、Android Lint、Debug/Release 构建及交互回归已经通过；厂商后台策略与长时运行仍需在受支持的真实设备上验收，因此暂不标记稳定版。
...
| --- | --- | --- | --- |
| Android 6.0（API 23）及以上 | 支持 | APK | `v0.3.0-beta.4`，待扩展真机矩阵 |
