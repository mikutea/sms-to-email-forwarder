...
> 当前版本：`v0.3.0-beta.2`（原生界面与后台可靠性预发布版）。自动化测试、Android Lint、Debug/Release 构建及 Android 15 虚拟设备短信回归已经通过；厂商后台策略与长时运行仍需在受支持的真实设备上验收，因此暂不标记稳定版。
...
- “雁笺”月白青玉视觉：统一折纸飞雁图标、轻量软拟态内容层、液态玻璃四项一级导航，以及遵循系统“减少动画”设置的流畅交互。

## 平台支持边界

| 平台 | 自动读取新短信 | 客户端形态 | 当前状态 |
| --- | --- | --- | --- |
| Android 6.0（API 23）及以上 | 支持 | APK | `v0.3.0-beta.2`，待扩展真机矩阵 |
