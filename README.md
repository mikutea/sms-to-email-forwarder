...
> 当前版本：`v0.3.0-beta.5`（交互一致性预发布版）。设置页九个功能入口已使用互不重复的目的地，心跳、隐私、配置迁移、平台能力和开源许可拥有独立页面；所有下拉选择统一为月白青玉玻璃表面。自动化测试、Android Lint、Debug/Release 构建及 Android 15 虚拟设备交互回归已经通过；厂商后台策略与长时运行仍需真机验收，因此暂不标记稳定版。
...
| --- | --- | --- | --- |
| Android 6.0（API 23）及以上 | 支持 | APK | `v0.3.0-beta.5`，待扩展真机矩阵 |
