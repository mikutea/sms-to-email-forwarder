...
> 当前版本：`v0.2.0-beta.1`（旅行守护测试版）。自动化测试、Android Lint、Debug/Release 构建已经通过；HUAWEI Mate 50 Pro / HarmonyOS 4.2 的锁屏、断网恢复和多日稳定性仍需真机验收，因此暂不标记稳定版。
...
- “雁笺”中国风视觉：玉色、纸色、朱砂色与飞雁印章图标；五页式简化导航。

## 平台支持边界

| 平台 | 自动读取新短信 | 客户端形态 | 当前状态 |
| --- | --- | --- | --- |
| Android 6.0（API 23）及以上 | 支持 | APK | `v0.2.0-beta.1`，待扩展真机矩阵 |
| HarmonyOS 2/3/4 Android 兼容环境 | 支持 | 与 Android 共用 APK | Mate 50 Pro / HarmonyOS 4.2 为首个验收目标 |
...
版本变化见 [CHANGELOG.md](CHANGELOG.md)。安全边界与报告方式见 [SECURITY.md](SECURITY.md)。本项目仅用于设备所有者明确授权的短信；部署者负责遵守适用的隐私、通信和数据保护要求。
