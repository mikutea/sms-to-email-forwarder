...
> 当前稳定版：`v1.0.0`；可选测试版：`v1.0.1-beta.1`。权限向导会优先调用系统的单应用确认界面，并在返回后读取真实状态；厂商启动管理无法由普通 App 自动授权，雁笺会尝试兼容入口并可靠回退到本应用详情。正式版与 Beta 使用同一签名，可覆盖安装并保留本机配置。
...
- 支持稳定版 / Beta 更新通道，每天最多检查一次项目官方 GitHub Release；下载和安装始终由用户确认。
