...
> 当前稳定版：`v1.0.0`；可选测试版：`v1.1.0-beta.1`。权限向导会优先调用系统的单应用确认界面，并在返回后读取真实状态；厂商启动管理无法由普通 App 自动授权，雁笺会尝试兼容入口并可靠回退到本应用详情。正式版与 Beta 使用同一签名，可覆盖安装并保留本机配置。
...
- 支持稳定版 / Beta 更新通道和应用内下载；安装前校验发布资产大小、SHA-256、包名、版本代码及当前正式签名，最终覆盖安装由系统和用户确认。
...
| --- | --- | --- | --- |
| Android 6.0（API 23）及以上 | 支持 | APK | `v1.0.0` |
...
4. 按“系统守护”逐项点按：短信权限和忽略电池优化会直接显示系统确认；厂商启动管理需要在打开的系统页中允许自启动、关联启动和后台活动。
