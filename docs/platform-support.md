...
因此，本项目不会通过通知监听、无障碍自动点击或私有 API 冒充 NEXT 短信接收支持。Android / 鸿蒙 2–4 客户端中可选的通知使用权只用于在 SMTP 成功后调用默认短信应用公开的“标记已读”动作，绝不作为短信正文来源。这些方案不能为 NEXT 提供可靠的短信接收能力，也不具备稳定的系统升级兼容性。
